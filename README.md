...
이 프로젝트는 **서버 비용을 최대한 줄일 수 있는 실시간 대전 구조**를 만들어보고 싶다는 생각에서 시작했습니다.

보통의 서버 권위 구조에서는 서버가 모든 경기 상태를 계산하고 플레이어에게 계속 브로드캐스트합니다. 안정적이지만 경기가 늘어날수록 서버의 연산과 네트워크 비용도 함께 증가합니다.
...
두 목표 모두 **서버를 거치지 않는 P2P 구조**라면 해결할 수 있을 것이라 생각했고, Lockstep을 기반으로 전체 구조를 설계했습니다.

---

⚠️ **새로운 문제**

서버가 경기 상태를 알 수 없게 되면서, 클라이언트가 보낸 경기 결과를 그대로 믿을 수밖에 없는 구조가 되었습니다.

그래서 프로젝트를 진행하면서 목표를 하나 더 추가했습니다.

🎯 **세 번째 목표**

**공정성** — 서버가 경기 결과를 독립적으로 검증할 수 있을 것

이 README는 **이 세 가지 목표를 구현하면서 부딪힌 문제와 해결 과정을 순서대로 정리한 내용**입니다.
...
- 서버가 경기 상태를 매 틱 계산하지 않기
- 입력이 서버를 거치지 않고 바로 상대에게 전달되기

낮은 서버 비용과 통신 지연 절감, 두 목표 모두 게임 트래픽이 서버를 거치지 않으면 해결되는 문제였습니다.

그래서 경기 중에는 **P2P 구조**를 사용하기로 했습니다.

서버는 매칭과 UDP 홀펀칭만 담당하고, 연결이 완료되면 더 이상 게임 데이터는 전달하지 않습니다.

전송 프로토콜은 TCP 대신 UDP를 선택했고, ACK와 재전송처럼 게임에 필요한 기능만 직접 구현했습니다.
...
- 서버는 경기 상태를 계산하지 않아도 되었고
- 상태를 브로드캐스트할 필요도 없어졌으며
- 입력도 서버를 거치지 않게 되었습니다.

하지만 문제 상황도 발생했습니다.

- UDP는 입력 하나만 유실되어도 두 클라이언트의 게임 상태가 달라질 수 있습니다. 
- 두 플레이어가 항상 같은 게임을 플레이하고 있다는 것을 어떻게 보장할까?
...
답은 평소 즐겨 하던 게임인 **스타크래프트**에서 찾았습니다.

예전부터 수백 개의 유닛이 동시에 움직이는 RTS가 어떻게 당시 컴퓨터와 네트워크 환경에서도 원활하게 멀티플레이를 지원할 수 있었는지 궁금했습니다.

얼마 전 스타크래프트가 **게임 상태가 아니라 플레이어의 입력만 주고받고**, 각 클라이언트가 **같은 로직을 같은 순서로 실행한다**는 사실을 알게 되었습니다.

제가 만들고 있던 게임도 입력만 주고받는 구조였기 때문에 이 방식을 그대로 적용해 보기로 했습니다.

### 구현

Lockstep을 적용하기 위해 시뮬레이션을 다음과 같이 구성했습니다.

- **입력만 전송** — 공과 패들의 좌표 대신 `위 / 아래 / 정지`
- **결정론 시뮬레이션** — 물리와 충돌은 모두 정수 연산
...
그래서 보낸 입력은 ACK를 받을 때까지 `unackedInputs`에 보관하고, 100ms마다 다시 전송하도록 만들었습니다.

입력이 유실되더라도 결국 같은 입력을 공유하게 됩니다.

### Input Delay

하지만 Lockstep에도 문제가 있었습니다.

양쪽 입력이 모두 도착해야 다음 틱을 실행하기 때문에, 상대 패킷이 조금만 늦어져도 게임이 멈춰 버렸습니다.

이를 해결하기 위해 **Input Delay**를 적용했습니다.

현재 입력을 바로 실행하지 않고 **현재 Tick + 3**에 예약해서 전송합니다.

실제로 그 틱이 실행될 때쯤에는 상대 입력도 대부분 도착해 있기 때문에 게임이 끊기는 현상을 크게 줄일 수 있었습니다.

> 📌 **Trade-off**
>
> 입력은 약간 늦게 반영되지만 게임의 플레이를 방해할 정도는 아닙니다.
>
> 게임이 멈췄다가 한꺼번에 진행되는 현상보다 훨씬 자연스러운 플레이를 얻을 수 있었습니다.
