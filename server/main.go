...
func main() {
	...
	for {
		...
		if len(peers) == 2 {
			...
			sendAddr(conn, peerA, peerB, 1)
			sendAddr(conn, peerB, peerA, 2)
...
func sendAddr(conn *net.UDPConn, to *net.UDPAddr, addr *net.UDPAddr, idx int) {
	var message string
  message = fmt.Sprintf("%s:%d", addr.IP.String(), addr.Port)

	_, err := conn.WriteToUDP(append([]byte{byte(idx)}, []byte(message)...), to)
	if err != nil {
